<?php
	...
	$update = $q->query($sql);
	if($update) {
		$sqlinput = "INSERT INTO tr_anggota (nik, nama, email, tmplahir, tgllahir, jenkel, alamat, telprumah, telphp, stsdaftar, nipp, instansi, gaji, is_aktif, username, passwordasli, user_id, role_id, created_at) SELECT nik, nama, email, tmplahir, tgllahir, jenkel, alamat, telprumah, telphp, stsdaftar, nipp, instansi, gaji, 1, '".$_POST['username']."', '".$_POST['password']."', '".$_SESSION['user_id']."', '".$_SESSION['role_id']."', '".$now."' FROM tr_calonanggota WHERE id = '".$id."'";
